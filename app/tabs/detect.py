...
def render_speedometer(real_prob: float) -> go.Figure:
    ...
    if score >= 75:
        needle_color, zone_label = "#16a34a", "Likely Authentic"
    elif score >= 50:
        needle_color, zone_label = "#eab308", "Uncertain"
    elif score >= 25:
        needle_color, zone_label = "#f97316", "Likely Fake"
    else:
        needle_color, zone_label = "#dc2626", "High Risk"
    ...
    fig = go.Figure(go.Indicator(
        ...
        number={
            "suffix": "%",
            "font": {"size": 48, "color": "#0f172a", "family": "Inter, sans-serif"},
        },
        title={
            "text": f"Authenticity Score<br><span style='font-size:14px;color:#64748b;font-weight:500'>{zone_label}</span>",
...
def render():
    ...
    if st.session_state["detect_result"] is not None:
        ...
        v_msg      = "This article shows strong indicators of misinformation." if is_fake else "This article appears to be authentic and credible."
        ...
        # Speedometer + signals side by side
        ...
        with col_signals:
            ...
            <div style='background:{card};border:1px solid {border};border-radius:12px;
                padding:20px;height:100%;box-shadow:0 2px 8px rgba(0,0,0,0.04);'>
                <p style='font-size:13px;font-weight:700;color:{text};margin:0 0 16px 0;
        ...
        <div style='background:{muted};border:1px solid {border};border-radius:12px;
            ...
            <p style='font-size:11px;font-weight:700;color:{sub};margin:0 0 10px 0;
                text-transform:uppercase;letter-spacing:0.8px;'>Article Analysed</p>
            <p style='font-size:13px;color:{text};margin:0;line-height:1.7;'>
                {inputs["article_text"][:500]}{'...' if len(inputs["article_text"]) > 500 else ''}
            </p>
        </div>
    ...
    with col_form:
        ...
        <div style='background:{card};border:1px solid {border};border-radius:14px;
            padding:28px 28px 24px 28px;box-shadow:0 2px 12px rgba(0,0,0,0.05);'>
            <h3 style='font-size:18px;font-weight:800;color:{text};margin:0 0 6px 0;'>
                Analyse an Article
            </h3>
            <p style='font-size:13px;color:{sub};margin:0 0 20px 0;'>
                Paste article text and fill in source details for the most accurate result.
            </p>
        </div>
        """, unsafe_allow_html=True)
        ...
        if char_count > 0:
            ok = char_count >= 200
            st.markdown(
                f"<p style='font-size:12px;color:{'#16a34a' if ok else '#f97316'};margin-top:-6px;'>"
    ...
    with col_info:
        ...
        <div style='background:{muted};border:1px solid {border};border-radius:12px;
            ...
            <p style='font-size:12px;font-weight:700;color:{text};margin:0 0 12px 0;
                text-transform:uppercase;letter-spacing:0.5px;'>How It Works</p>
            <div style='display:flex;flex-direction:column;gap:12px;'>
                <div style='display:flex;gap:10px;align-items:flex-start;'>
                    <span style='font-size:16px;flex-shrink:0;'>📝</span>
                    <p style='font-size:12px;color:{sub};margin:0;line-height:1.5;'>
                        Paste any article text — news, social media posts, blog content.
                    </p>
                </div>
                <div style='display:flex;gap:10px;align-items:flex-start;'>
                    <span style='font-size:16px;flex-shrink:0;'>🔗</span>
                    <p style='font-size:12px;color:{sub};margin:0;line-height:1.5;'>
                        Add source details to improve accuracy with social context signals.
                    </p>
                </div>
                <div style='display:flex;gap:10px;align-items:flex-start;'>
                    <span style='font-size:16px;flex-shrink:0;'>🤖</span>
                    <p style='font-size:12px;color:{sub};margin:0;line-height:1.5;'>
                        DistilBERT encodes the text. XGBoost classifies using 773 features.
                    </p>
                </div>
                <div style='display:flex;gap:10px;align-items:flex-start;'>
                    <span style='font-size:16px;flex-shrink:0;'>📊</span>
                    <p style='font-size:12px;color:{sub};margin:0;line-height:1.5;'>
                        Get a verdict with confidence score and signal breakdown.
                    </p>
                </div>
            </div>
